...
function group2(data) {
    ...
    this.go = function() {
        ...
        document.getElementById('leftCont2').innerHTML = renderLeft2(baseData2, baseDom2); // 渲染左侧
        chooseParent2(); // 选中父级
...
function renderLeft2(data, dom) {
	...
	if(Object.prototype.toString.call(data) === '[object Array]') {
		for(var i = 0; i < data.length; i++) {
            ...
            var arrow = '<span class="to__dropdownList" ><i src="static/smc/img/right-arrow.png" onclick="dropClick2(this)"></i></span>';
            if (data[i].last == true) {
                arrow = '';
            }
            ...
            var defaultImg = '';
            if (data[i].last == true) {
                defaultImg = data[i].img || 'static/smc/img/header-icon.png';
            } else {
                defaultImg = data[i].img || 'static/smc/img/icon-folder.svg';
            }
            var prefix = '<span class="to__prefix"><img src="' + defaultImg + '"></img></span>';

			// 渲染dom
			//          item.innerHTML = '<div class="weui-cell to__item " id="group2_' + data[i].group2 + '">' + arrow + '<span class="member ' + memberDisabled + '" onclick="nodeClick2(this)"><input id="input'+ data[i].group2 +'" ' + disabled + ' type="checkbox"  ' + checked + '" name="cName"  value="' + data[i].name + '" onclick="checkboxClick2(this)" />' + prefix + '<div class="text"><div class="to__name">' + data[i].name + '</div><div class="to__post">' +data[i].post + '</div></div>' + numberDom + '<div class="to__parentName">' + parentName + '</div><span class="icon__round ' + iconClass + '"></span></span></div>';
			item.innerHTML = '<div class="weui-cell to__item " id="group2_' + data[i].group2 + '">' + arrow + '<span class="member ' + memberDisabled + '" onclick="nodeClick2(this)"><input :disabled = "data[i].disabled === true" id="input' + data[i].group2 + '" type="checkbox"  ' + checked + '" name="cName"  value="' + data[i].name + '" onclick="checkboxClick2(this)" />' + prefix + '<div class="text"><div class="to__name">' + data[i].name + '</div><div class="to__post">' + data[i].post + '</div></div>' + numberDom + '<div class="to__parentName">' + parentName + '</div><span class="icon__round ' + iconClass + '"></span></span></div>';
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (item.children.length > 0 && item.last != true) {
            ...
            if ((item.last == true || oldSearch2) && item.active == true) {
                ...
                var defaultImg = '';
                defaultImg =  item.img  || 'static/smc/img/header-icon.png';
                div.innerHTML = '<span class="to__prefix"><img src="' + defaultImg + '"><input  type="hidden" name="sendUser" value="' + item.value + '" /></img></span><span class="to__name render" id="render' + item.group2 + '" >' + item.name + '</span><div class="to__parentName" style="display:none"></div><span id="to__close" class="to__close" onclick="nodeCencel2(this)"><i></i></span>';
...
function updateNode2(name) {
    ...
    document.getElementById('leftCont2').innerHTML = renderLeft2(data, baseDom2); // 渲染左侧
}
